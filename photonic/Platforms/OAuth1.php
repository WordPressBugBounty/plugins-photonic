...
abstract class OAuth1 extends Base {
	...
	protected function generate_signature($api_call = null, $api_args = null, $method = null, $oauth_token = []): string {
		$encoded_key = self::urlencode_rfc3986($this->api_secret) . '&';
		if (isset($oauth_token['oauth_token_secret'])) {
			$encoded_key .= self::urlencode_rfc3986($oauth_token['oauth_token_secret']);
		}
		...
		ksort($params);
		$string      = self::build_query($params);
		$base_string = $method . '&' . self::urlencode_rfc3986($end_point) . '&' . self::urlencode_rfc3986($string);
